    ...
    methods:{
        init: function(){
            ...
            for (let i = 0; i < models.length; i++) {
                ...
                if(fs.lstatSync(PathManager.MODELPATH + models[i]).isDirectory()){
                    ...
                    for(let j = 0; j < childF.length; j++){
                        ...
                        if(window.path.extname(childF[j]).toLowerCase() == '.pmx' || window.path.extname(childF[j]).toLowerCase() == '.pmd' || window.path.extname(childF[j]).toLowerCase() == '.x'){
                            d.models.push("/data/Model/" + models[i] + '/' + childF[j]);
                        }
                    }
                }
            ...
            for (let i = 0; i < mmes.length; i++) {
                let d = {"id":i,"name":mmes[i],"address":PathManager.MMEPATH + mmes[i] + "\\"}
                window.fs.exists(PathManager.MMEPATH + mmes[i] + "\\info.json", function(exists){
                    if(exists){
                        window.fs.readFile(PathManager.MMEPATH + mmes[i] + "\\info.json", (err, data) => {
                            var rf = data.toString('utf8');
                            var rj = JSON.parse(data.toString('utf8'));
                            //console.log(rj)
                            d['info'] = rj
            ...
            for (let i = 0; i < vmds.length; i++) {
                let d = {"id":i,"name":vmds[i],"address":PathManager.VMDPATH + vmds[i] + "\\"}
                window.fs.exists(PathManager.VMDPATH + vmds[i] + "\\info.json", function(exists){
                    if(exists){
                        window.fs.readFile(PathManager.VMDPATH + vmds[i] + "\\info.json", (err, data) => {
                            var rf = data.toString('utf8');
                            var rj = JSON.parse(data.toString('utf8'));
                            //console.log(rj)
                            d['info'] = rj
                ...
                window.fs.exists(PathManager.VMDPATH + vmds[i] + "\\image.gif", function(exists){
                    if(exists){
                        d.img = 'yes'
                    }
                    else{
...
var componentIndex = {
    ...
    methods:{
        ...
        getVMDNum: function(){
            return this.vmds.length
        },
        getMMENum: function(){
            return this.mmes.length
        },
